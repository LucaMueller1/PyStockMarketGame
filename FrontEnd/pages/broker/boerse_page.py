    ...
    if mode_switch == "Buy":
        if session_state.stock_desc:
            index = [
                ...
                if session_state.stock_desc["symbol"] in s
            ][0] + 1
        else:
            index = 0
        ...
        if ticker_code_entry_raw != " ":
            ...
            with col1:
                ...
                st.write(
                    """<div class="markdown-text-container stMarkdown" style="width: 349px;"><p>Chosen Quantity: <b><code style="color: black;">"""
                    + str(ticker_quantity_entry)
                    + """</code></b></p></div> """,
                    unsafe_allow_html=True,
                )
                st.markdown(
